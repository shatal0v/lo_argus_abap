...
*"* do not include other source files here!!!
  ...
  methods SET_TAG_TABLE
    importing
      !IT_TABLE type STANDARD TABLE
      !IV_ESCAPE type FLAG default 'X'
      !IV_AREA_NAME type PSTRING optional
      !IV_SHIFT type FLAG optional .
  methods SET_TAG_VALUE
    importing
      !IV_TAG type PSTRING
      !IV_VALUE type PSTRING .
  methods ADD_COMMENT
    importing
...
METHOD insert_after.
  DATA: lt_sht TYPE TABLE OF zexcel_s_cell_data.
  lt_sht = sheet_content.
  SORT lt_sht BY cell_row DESCENDING.
  LOOP AT lt_sht ASSIGNING FIELD-SYMBOL(<ls_sht>) WHERE cell_row > ip_row.
    TRY.
        set_cell(
          EXPORTING
            ip_column    = <ls_sht>-cell_column
            ip_row       = <ls_sht>-cell_row + 1
            ip_value     = <ls_sht>-cell_value
            ip_style     =  <ls_sht>-cell_style
            ip_data_type = <ls_sht>-data_type
        ).
        DATA(lv_row) = get_row( ip_row = <ls_sht>-cell_row ).
        DATA(lv_height) = lv_row->get_row_height( ).
        TRY.
            get_row( ip_row = <ls_sht>-cell_row + 1 )->set_row_height( ip_row_height = lv_height ).
          CATCH zcx_excel.
        ENDTRY.
        set_cell(
          EXPORTING
            ip_column    = <ls_sht>-cell_column
            ip_row       = <ls_sht>-cell_row
            ip_value     = VALUE pstring( )
...
METHOD set_tag_table.
  DATA: lt_sheet_desc TYPE TABLE OF zexcel_s_cell_data.
  DATA(lv_main_row) = VALUE int4( ).
  CHECK it_table IS NOT INITIAL.
  ASSIGN it_table[ 1 ] TO FIELD-SYMBOL(<ls_tab>).
  DATA(lt_comp) = CAST cl_abap_structdescr( CAST cl_abap_tabledescr( cl_abap_typedescr=>describe_by_data( it_table ) )->get_table_line_type( ) )->get_components( ).
  LOOP AT lt_comp INTO DATA(ls_comp).
    if iv_area_name IS NOT INITIAL.
      ls_comp-name = |{ iv_area_name }-{ ls_comp-name }|.
    ENDIF.
    IF iv_escape = abap_true.
      ls_comp-name = |[{ ls_comp-name }]|.
    ENDIF.
    LOOP AT sheet_content ASSIGNING FIELD-SYMBOL(<ls_sheet>) WHERE cell_value CS ls_comp-name.
      lv_main_row = <ls_sheet>-cell_row.
      EXIT.
    ENDLOOP.
    IF sy-subrc = 0.
      DATA(lt_main_row) = VALUE zexcel_t_cell_data( FOR ls_con IN sheet_content WHERE ( cell_row = lv_main_row ) ( ls_con ) ).
      EXIT.
    ENDIF.
  ENDLOOP.
  CHECK lv_main_row > 0.
  IF iv_shift = abap_true.
    DO lines( it_table ) - 1 TIMES.
      lt_sheet_desc = sheet_content.
      SORT lt_sheet_desc BY cell_row DESCENDING.
      DELETE lt_sheet_desc WHERE cell_row <= lv_main_row.
      LOOP AT lt_sheet_desc ASSIGNING <ls_sheet>.
        TRY.
            set_cell(
                ip_column    = <ls_sheet>-cell_column
                ip_row       = <ls_sheet>-cell_row + 1
                ip_value     = <ls_sheet>-cell_value
                ip_formula   = <ls_sheet>-cell_formula
                ip_style     = <ls_sheet>-cell_style
                ip_data_type = <ls_sheet>-data_type
            ).
          CATCH zcx_excel.
        ENDTRY.
      ENDLOOP.
  ...
  LOOP AT it_table ASSIGNING FIELD-SYMBOL(<ls_pl>).
    LOOP AT lt_main_row ASSIGNING FIELD-SYMBOL(<ls_content>).
      TRY.
          set_cell(
              ip_column    = <ls_content>-cell_column
              ip_row       = lv_main_row + lv_tabix
    ...
    LOOP AT lt_comp INTO ls_comp.
      ASSIGN COMPONENT ls_comp-name OF STRUCTURE <ls_pl> TO FIELD-SYMBOL(<lv_fld>).
      IF sy-subrc = 0.
        if iv_area_name IS NOT INITIAL.
          ls_comp-name = |{ iv_area_name }-{ ls_comp-name }|.
        ENDIF.
        IF iv_escape = abap_true.
          ls_comp-name = |[{ ls_comp-name }]|.
        ENDIF.
        LOOP AT sheet_content ASSIGNING <ls_sheet> WHERE cell_value CS ls_comp-name.
          <ls_sheet>-cell_value = replace( case = abap_false val = <ls_sheet>-cell_value sub = ls_comp-name with = <lv_fld> occ = 0 ).
        ENDLOOP.
      ENDIF.
    ENDLOOP.
  ENDLOOP.
ENDMETHOD.
  ...
  METHOD set_tag_value.
    LOOP AT sheet_content ASSIGNING FIELD-SYMBOL(<ls_sheet>) WHERE cell_value CS iv_tag.
      <ls_sheet>-cell_value = replace( case = abap_false val = <ls_sheet>-cell_value sub = iv_tag with = iv_value occ = 0 ).
    ENDLOOP.
  ENDMETHOD.
